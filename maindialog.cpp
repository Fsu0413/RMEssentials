...
void MainDialog::selectFolder()
{
    ...
    bool bhasImd = existImds(d);

    if (!(bhasMp3 && bhasBigPng && bhasImd)) {
...
void MainDialog::checkFiles(const QString &folder)
{
    ...
    QDir d(folder);
    bool exists[12] = {false};
    exists[0] = hasMp3(d);
    exists[1] = hasBigPng(d);
    const char *smallPngSuffix = NULL;
    exists[2] = hasSmallPng(d, smallPngSuffix);

    ExistImds imds = existImds(d);
    exists[3] = imds & IMD_4K_EZ;
    exists[4] = imds & IMD_4K_NM;
    exists[5] = imds & IMD_4K_HD;
    exists[6] = imds & IMD_5K_EZ;
    exists[7] = imds & IMD_5K_NM;
    exists[8] = imds & IMD_5K_HD;
    exists[9] = imds & IMD_6K_EZ;
    exists[10] = imds & IMD_6K_NM;
    exists[11] = imds & IMD_6K_HD;
